...
function Get-AzIdentityAndRBAC {
    [CmdletBinding()]
    param(
    ...
    try {
        ...
        # Use provided SubscriptionId or current context
        if (-not $SubscriptionId) {
            $SubscriptionId = $azIdentity.id
        }
        if (-not $UserId) {
            $UserContext = az ad signed-in-user show -o json | ConvertFrom-Json
        }

        # Get direct RBAC assignments
        $rbacAssignments = az role assignment list --subscription $SubscriptionId --assignee $($UserContext.id) -o json | ConvertFrom-Json
        if ($rbacAssignments) {
            $roles = $rbacAssignments | Select-Object -ExpandProperty roleDefinitionName -Unique
            Write-Host "Direct RBAC Assignment: $($roles -join ', ')"
        }
        else {
            Write-Host "No Direct RBAC assignments found for user $($UserContext.userPrincipalName)"
            Write-Host ""
            if ($identityType -eq 'user') {
                # Check group memberships and inherited RBAC
                $userGroups = az ad user get-member-groups --id $($UserContext.id) --security-enabled-only -o json | ConvertFrom-Json
                $rbacAssignments = az role assignment list --include-groups --include-inherited --subscription $SubscriptionId --query "[?roleDefinitionName=='Owner' || roleDefinitionName=='Contributor']" | ConvertFrom-Json
                $groupIdsWithRBAC = @()
                foreach ($group in $userGroups) {
                    $matched = $false
                    foreach ($assignment in $rbacAssignments) {
                        if ($assignment.principalType -eq "Group" -and $assignment.principalId -eq $group.id) {
                            if (-not $matched) {
                                Write-Host "Found RBAC assignment: $($group.displayName), $($group.id)"
                                $matched = $true
                            }
                            $groupIdsWithRBAC += $group
                        }
                    }
                }
                if ($groupIdsWithRBAC.Count -gt 0) {
                    $groupInfo = $groupIdsWithRBAC | ForEach-Object { "$($_.displayName) (ID: $($_.id))" } | Join-String -Separator ", "
                    Write-Host "User $($UserContext.userPrincipalName) has RBAC assignments via group(s): $groupInfo"
                }
                else {
                    Write-Host "User $($UserContext.userPrincipalName) does not have RBAC assignments via group membership"
                    break
                }
            }
        }
...
$azIdentityName = Get-AzIdentityAndRBAC

$publicIp = Invoke-RestMethod -Method 'Get' -Uri "https://ifconfig.me/ip" -UseBasicParsing

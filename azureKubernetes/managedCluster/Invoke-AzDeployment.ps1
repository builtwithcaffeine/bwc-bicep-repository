...
Write-Output `r "Checking AKS Admins Security Group exists"
$groupName = "aks-admin-$customerName-$environmentType-$location"

# Try to get the group first
$aksAdminsEntraGroup = az ad group list --filter "displayName eq '$groupName'" --output json | ConvertFrom-Json | Select-Object -First 1

if (-not $aksAdminsEntraGroup) {
    Write-Output "Group not found. Creating AKS Admins Security Group: [$groupName]"
    $aksAdminsEntraGroup = az ad group create --display-name $groupName --mail-nickname $groupName --output json | ConvertFrom-Json
} else {
    Write-Output "Group: [$groupName] already exists. Using existing group."
}
...
if ($deploy) {
    ...
    az deployment $targetScope create `
        ...
        aksAdminsGroupId=$($aksAdminsEntraGroup.id) `
